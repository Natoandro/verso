...
pub fn parseBool(value: []const u8) Error!bool {
    if (std.mem.eql(u8, value, "true")) return true;
    if (std.mem.eql(u8, value, "false")) return false;
    return error.InvalidValue;
}

pub fn parseUnsigned(comptime T: type, value: []const u8) Error!T {
    return std.fmt.parseUnsigned(T, value, 10) catch error.InvalidValue;
}

pub fn parseEnum(comptime T: type, value: []const u8) Error!T {
